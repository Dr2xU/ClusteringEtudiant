...
class StudentVote(db.Model):
    ...
    voter_id = db.Column(db.Integer, db.ForeignKey('students.id'), nullable=False)

    # Foreign key to the student who is being voted for
    candidate_id = db.Column(db.Integer, db.ForeignKey('students.id'), nullable=False)
    ...
    voter = db.relationship('Student', foreign_keys=[voter_id], backref=db.backref('votes_cast', lazy=True))
    candidate = db.relationship('Student', foreign_keys=[candidate_id], backref=db.backref('votes_received', lazy=True))
